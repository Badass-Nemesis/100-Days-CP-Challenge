// Question -> https://codeforces.com/contest/895/problem/A
...
public class Qes1 {
    ...
    public static void solve(int n) throws IOException {
        ...
        int sum = 0;
        int ans = 360; // because 360 max hota h, agar isse chota milega to update karenge
        int leftPointer = 0;
        int rightPointer;
        ...
        for (rightPointer = 0; rightPointer < n; rightPointer++) {
            ...
            while (sum >= 180) {
                // System.out.println(leftPointer + " " + sum + " " + rightPointer);
                ans = Math.min(ans, 2 * (Math.abs(sum - 180)));
                // important point. sabse pehle ans check karo, uske baad hi leftPointer badhao
                // aur sum minus karo
                sum -= arr[leftPointer];
                leftPointer++;
            }
