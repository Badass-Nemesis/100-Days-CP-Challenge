...
public class Qes1 {
    ...
    public static void solve() throws IOException {
        ...
        for (int i = 1; i <= n; i++) {
            int temp = in.nextInt();
            arr[temp] = i; // number ko index maan ki array mein number ka latest.max index store kar rahe
                           // h
        }
            ...
            if (arr[i] != -1) {
                for (int j = 1; j < 1001; j++) {
                    if (arr[j] != -1 && gcd(i, j) == 1) {
                        ans = Math.max(ans, arr[i] + arr[j]); // taking max of ans and sum of index
                    }
                }
            }
        }
    ...
    // this solve2 (my sol-n) is getting TLE. WHAT THE FUQ?!
    public static void solve2() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            arr[i] = in.nextInt();
            hm.put(arr[i], i); // isse max index mil jayega wo element ka
        }
        ...
        for (int i = 0; i < n; i++) {
            if (arr[i] != -1) {
                for (Integer key : hm.keySet()) {
                    if (arr[hm.get(key)] != -1 && gcd(arr[i], key) == 1) {
                        ans = Math.max(ans, i + hm.get(key) + 2);
                    }
                }
            }
        }
